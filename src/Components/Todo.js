...
export default function Todo() {
    ...
    return ( < >
            ...
            id = "list1"
            style = {
                { borderRadius: .75 + "rem", backgroundColor: "#f8f9fa" } } >
            <
            ...
            /p> <
            p style = {
                { textAlign: 'center', marginTop: -1 + 'rem' } } > { todos.length }
            tasks pending! < /p> <
            form >
            <
            div className = "pb-2" >
...

            {
                todos.map((todo => ( < >
                            ...
                            onClick = { e => firestore.collection('Todos').doc(currentUser.uid).collection('Todos').doc(todo.id).delete() } > < TrashFill / > < /button>
